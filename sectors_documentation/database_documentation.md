...
| `affiliates` | text[] | | The list of the company's affiliates (if any) conglomerates groups |
...
| `foreign_sell_volume` | int8 | | The number of shares sold by foreign investors in that day |
| `foreign_buy_volume` | int8 | | The number of shares bought by foreign investors in that day |
| `updated_on` | timestamptz | | The time when the data is updated or pushed into the database |
...
| `payment_date`      | date        |             | The date when the dividend payment is transferred from the company to investor |
...
### idx_comprehensive_report

----------------------

This materialized view serves as a centralized, denormalized master record for each listed company. It consolidates price performance, valuation metrics, real-time market data, detailed historical financial statements (annual and quarterly), corporate actions (dividends, splits, rights issues), and insider trading activities into a single row per symbol for efficient data retrieval. This MV get scheduled refresh everyday at 05:15 (GMT+7).
...
| ebitda_ttm                        | bigint           |             | Earnings Before Interest, Taxes, Depreciation, and Amortization over the trailing twelve months. Sum of ebitda for the last 4 quarters from [idx_combine_financials_quarterly](#idx_combine_financials_quarterly) or fallback to [idx_calc_metrics_quarterly](#idx_calc_metrics_quarterly).                                |
| net_income_ttm                    | bigint           |             | Net profit or loss over the trailing twelve months. Sum of earnings for the last 4 quarters from [idx_combine_financials_quarterly](#idx_combine_financials_quarterly).                                                                                                                                                                                      |
...
| price_change_1w                   | double precision |             | Percentage change in stock price over the last 1 week.price_change_7_days taken from [idx_calc_metrics_daily](#idx_calc_metrics_daily).                                                                                                                                                                                                                     |
...
| dividend_list                     | jsonb            |             | Object keyed by year containing arrays of dividend payment details. Aggregated JSON constructed from [idx_dividend](#idx_dividend).                                                                                                                                                                                                                          |
| stock_split_list                  | jsonb            |             | Array of objects detailing historical stock split actions. Aggregated JSON constructed from [idx_stock_split](#idx_stock_split).                                                                                                                                                                                                                             |
| right_issue_list                  | jsonb            |             | Array of objects detailing historical right issue actions. Aggregated JSON constructed from [idx_right_issue](#idx_right_issue).                                                                                                                                                                                                                             |
| upcoming_dividend_list            | jsonb            |             | Array of objects detailing declared dividends that have not yet been paid. Aggregated JSON constructed from [idx_upcoming_dividend](#idx_upcoming_dividend).                                                                                                                                                                                                 |
| warrant_list                      | jsonb            |             | Array of objects detailing warrant issuances and trading periods. Aggregated JSON constructed from [idx_warrant](#idx_warrant).                                                                                                                                                                                                                              |
| historical_revenue_opex_breakdown | jsonb            |             | Array of objects detailing the breakdown of revenue sources and operating expenses per financial year. Aggregated JSON constructed from [idx_manual_input](#idx_manual_input).                                                                                                                                                                               |
...
This materialized view consolidates and summarizes key financial, market, and profile data for each active company listed on the Singapore Exchange (SGX). It prioritizes manually curated data from [sgx_manual_input](#sgx_manual_input) over automated data from [sgx_companies](#sgx_companies) to ensure the highest accuracy. The view calculates various performance metrics using [sgx_daily_data](#sgx_daily_data), aggregates historical financials, and provides a comprehensive report for each company. It is scheduled to refresh twice daily.

|                        |                  |                                                                                                                                                                                                                                                                                                                           |
| ---------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| column_name            | data_type        | Description                                                                                                                                                                                                                                                                                                               |
| name                   | text             | Full name of the company. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                                                              |
| market_cap             | bigint           | The total market value of the company's outstanding shares. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                            |
| volume                 | double precision | The number of shares traded on the last finished trading day. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                          |
| pe                     | double precision | Price-to-Earnings (P/E) ratio for the trailing twelve months (TTM). Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                    |
| eps                    | double precision | Earnings Per Share for the trailing twelve months (TTM). Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                               |
| beta                   | double precision | A measure of the stock's volatility in relation to the overall market. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                 |
| change_1d              | numeric          | The percentage change in the stock's closing price from the previous trading day. Calculated from [sgx_daily_data](#sgx_daily_data).                                                                                                                                                    |
| change_7d              | numeric          | The percentage change in the stock's closing price over the last 7 days. Calculated from [sgx_daily_data](#sgx_daily_data).                                                                                                                                                             |
| change_1m              | numeric          | The percentage change in the stock's closing price over the last 1 month. Calculated from [sgx_daily_data](#sgx_daily_data).                                                                                                                                                            |
| change_ytd             | double precision | The percentage change in the stock's price from the beginning of the current year. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                     |
| change_1y              | double precision | The percentage change in the stock's price over the last 1 year. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                       |
| change_3y              | double precision | The percentage change in the stock's price over the last 3 years. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                      |
| ps                     | double precision | Price-to-Sales (P/S) ratio for the trailing twelve months (TTM). Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                       |
| pcf                    | double precision | Price-to-Cash-Flow (P/CF) ratio for the trailing twelve months (TTM). Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                  |
| pb                     | double precision | Price-to-Book (P/B) ratio. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                                                             |
| gross_margin           | double precision | The percentage of revenue that exceeds the cost of goods sold. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                         |
| operating_margin       | double precision | The percentage of revenue remaining after covering operating expenses. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                 |
| net_profit_margin      | double precision | The percentage of revenue that translates into net profit. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                             |
| quick_ratio            | double precision | A liquidity ratio based on the most recent quarter (MRQ). Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                              |
| current_ratio          | double precision | A liquidity ratio based on the most recent quarter (MRQ). Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                              |
| debt_to_equity         | double precision | A leverage ratio based on the most recent quarter (MRQ). Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                               |
| dividend_yield_5y_avg  | double precision | The average dividend yield over the past five years. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                                   |
| dividend_growth_rate   | double precision | The annualized percentage rate of growth of a company's dividend. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                      |
| payout_ratio           | double precision | The proportion of earnings paid out as dividends. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                                      |
| sector                 | text             | The broad industry sector the company operates in. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                                     |
| sub_sector             | text             | A more specific industry sub-sector classification. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                                    |
| symbol                 | text             | The unique stock ticker symbol for the company on the SGX. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                             |
| close                  | jsonb            | A JSON dictionary object containing historical daily closing prices ({"YYYY-MM-DD": price}). Data aggregated from [sgx_daily_data](#sgx_daily_data).                                                                                                                                    |
| sgx_filings            | jsonb            | A JSON array of insider trading and disclosure filings from the last 90 days. Data aggregated from [sgx_filings](#sgx_filings).                                                                                                                                                         |
| sgx_buybacks           | jsonb            | A JSON array of share buyback transactions from the last 90 days. Data aggregated from [sgx_buybacks](#sgx_buybacks).                                                                                                                                                                   |
| employee_num           | double precision | The total number of employees. Prioritizes data from [sgx_manual_input](#sgx_manual_input), falling back to [sgx_companies](#sgx_companies).                                                                                                          |
| earnings               | bigint           | Net income for the trailing twelve months (TTM). Derived from [sgx_companies](#sgx_companies) historical earnings array.                                                                                                                                                                |
| revenue                | bigint           | Total revenue for the trailing twelve months (TTM). Derived from [sgx_companies](#sgx_companies) historical revenue array.                                                                                                                                                              |
| forward_dividend       | double precision | The estimated annual dividend per share for the next 12 months. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                        |
| forward_dividend_yield | double precision | The estimated dividend yield for the next 12 months. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                                   |
| dividend_ttm           | double precision | The total dividends per share paid over the trailing twelve months. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                    |
| historical_dividends   | jsonb            | A JSON dictionary object containing historical dividend data keyed by year ({"Year": { "total_dividend": ..., "total_yield": ...}}). Data taken from [sgx_companies](#sgx_companies).                                                                                                   |
| all_time_price         | jsonb            | A JSON dictionary object containing historical price milestones ({"key": {"date": ..., "price": ...}}). Data taken from [sgx_companies](#sgx_companies).                                                                                                                                |
| one_year_eps_growth    | double precision | The projected percentage growth in earnings per share for the next year. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                               |
| one_year_sales_growth  | double precision | The projected percentage growth in sales (revenue) for the next year. Data taken from [sgx_companies](#sgx_companies).                                                                                                                                                                  |
| historical_financials  | jsonb            | A JSON dictionary object keyed by year ({"Year": {object}}) containing annual financial data. It combines and prioritizes manually verified data from [sgx_manual_input](#sgx_manual_input) with automated data from [sgx_companies](#sgx_companies). |
